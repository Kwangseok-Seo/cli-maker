...
위 약속들은 문서로만 있지 않습니다. `go test ./...` 가 서브테스트 37개로 4xx 의 **출력 순서**(ADR-0005), 토큰이 없을 때의 **익명 전송**(ADR-0006), 터미널 여부에 따른 **기본 포맷**(ADR-0008)을 확인합니다. HTTP 는 목(mock) 없이 `httptest` 로 진짜 서버를 띄워, 서버가 실제로 받은 경로·쿼리·헤더로 판정합니다.
