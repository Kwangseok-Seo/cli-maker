...
## 테스트에서 — `t.Setenv`, 그리고 "없는 상태" 만들기

```go
t.Setenv("CLI_MAKER_TEST_TOKEN", "sekret")   // 테스트가 끝나면 원래대로 자동 복원
```

그런데 **"설정 안 됨"을 만드는 API 는 없다.** 위의 `Getenv`/`LookupEnv` 구별을 테스트하려면 그 상태가 필요한데:

```go
t.Setenv(k, "지워질 값")   // ① 복원 예약을 먼저 걸고
os.Unsetenv(k)            // ② 지운다
```

`t.Setenv` 가 등록한 복원은 테스트가 끝날 때 실행되므로, 개발자 셸에 그 변수가 **이미 있어도** 케이스가 흔들리지 않고 끝나면 원상복구된다([[go-test]]).
