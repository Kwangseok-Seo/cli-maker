...
## 값이 하나가 아니면 override 가 아니다

위 세 층은 **하나의 값**을 고르는 문제다 — 이길 층을 정하면 끝난다. 그런데 매니페스트 발견 경로처럼 각 층이 **여럿**을 담고 있으면 같은 규칙이 안 맞는다.

| | 뜻 | 예 |
|---|---|---|
| **override** | 첫 층 하나만 쓰고 나머지는 버린다 | `--timeout` |
| **merge** | 전부 읽어 합치고, **이름이 겹칠 때만** 순서가 개입한다 | `$PATH`, 매니페스트 발견([ADR-0013](../../adr/0013-manifest-discovery-paths.md)) |

"우선순위"라는 한 단어가 두 가지를 뜻해서, 앞 절의 사슬을 그대로 가져다 쓰면 프로젝트에 `apis/` 를 만드는 순간 전역 매니페스트가 통째로, 그것도 stderr 한 줄 없이 사라진다. 층에 무엇이 들어 있는지를 먼저 봐야 한다 → [[user-config-dir]]
